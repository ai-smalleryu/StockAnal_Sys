...
### 6.1 架构概览
```
用户请求 → Flask路由 → CoordinatorAgent
                          ↓ (LangGraph StateGraph)
         ┌────────────────┼────────────────┐
         ↓                ↓                ↓
    技术分析Agent    基本面Agent    资金流Agent
         ↓                ↓                ↓
         └────────────────┼────────────────┘
                          ↓
                   情绪/新闻Agent
         ...
         看多研究员              看空研究员
              └───────────┼───────────┘
                  ...
                  投资者人格分析(×4)
                          ↓
                   投资决策Agent → BUY/SELL/HOLD
```

### 6.2 数据共享
- **LangGraph SharedState**: Agent间状态共享
- **Redis统一缓存**: 数据层缓存
- **EventBus**: Agent间事件通信
- **AgentMemory**: 分析历史和经验学习

### 6.3 特性开关
- `USE_AGENT_SYSTEM=true`: 启用新Agent系统
- `USE_AGENT_SYSTEM=false`: 回退到旧TradingAgents
